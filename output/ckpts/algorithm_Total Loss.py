...
def dkd_loss(student_logits: torch.Tensor,
    ...
    Decouples classical KD into two independent components:
    - TCKD: Target Class Knowledge Distillation (binary KL on target vs non-target)
    - NCKD: Non-Target Class Knowledge Distillation (KL among non-target classes)
    
    In classical KD, NCKD weight is implicitly (1 - p_t^T), coupling it to TCKD.
    DKD uses independent hyperparameters alpha and beta.
    ...
    Args:
        ...
        labels: (B,) ground truth class indices
        T: temperature for distillation
        alpha: weight for TCKD component
        beta: weight for NCKD component
    
    Returns:
        DKD_loss: scalar tensor
    """
    # ── TCKD ──────────────────────────────────────────────────────────────────
    batch_size = student_logits.size(0)
    
    # Compute softmax probabilities at temperature T
    student_probs = F.softmax(student_logits / T, dim=1)   # (B, C)
    teacher_probs = F.softmax(teacher_logits / T, dim=1)   # (B, C)
    
    # Extract target class probabilities p_t
    arange = torch.arange(batch_size, device=student_logits.device)
    student_pt = student_probs[arange, labels]   # (B,)
    teacher_pt = teacher_probs[arange, labels]   # (B,)
    
    # Binary distributions: [p_t, 1 - p_t]
    student_binary = torch.stack([student_pt, 1.0 - student_pt], dim=1).clamp(min=1e-8)  # (B, 2)
    teacher_binary = torch.stack([teacher_pt, 1.0 - teacher_pt], dim=1).clamp(min=1e-8)  # (B, 2)
    ...
    # Mask out target class with -inf
    mask = torch.zeros(batch_size, num_classes, device=student_logits.device, dtype=torch.bool)
    ...
    # Softmax over non-target classes
    teacher_phat = F.softmax(teacher_logits_masked, dim=1)   # (B, C)
    student_log_phat = F.log_softmax(student_logits_masked, dim=1)  # (B, C)
    
    # Handle 0 * (-inf) = nan at target positions
    student_log_phat = torch.nan_to_num(student_log_phat, nan=0.0, neginf=0.0)
...
if __name__ == "__main__" or True:
    ...
    loss = dkd_loss(student_logits, teacher_logits, labels, T=T, alpha=alpha, beta=beta)
    
    assert loss.shape == torch.Size([]), f"Expected scalar, got shape {loss.shape}"
    assert loss.item() >= 0, f"DKD loss should be non-negative, got {loss.item()}"
    
    # Verify linearity: DKD(alpha=1, beta=0) should equal TCKD only
    loss_tckd_only = dkd_loss(student_logits, teacher_logits, labels, T=T, alpha=1.0, beta=0.0)
    loss_nckd_only = dkd_loss(student_logits, teacher_logits, labels, T=T, alpha=0.0, beta=1.0)
    loss_combined = dkd_loss(student_logits, teacher_logits, labels, T=T, alpha=2.0, beta=3.0)
    expected_combined = 2.0 * loss_tckd_only + 3.0 * loss_nckd_only
    assert torch.allclose(loss_combined, expected_combined, atol=1e-5), \
        f"Linearity check failed: {loss_combined.item()} != {expected_combined.item()}"
    
    print(f"ALGO_CHECK DKD (Decoupled Knowledge Distillation): OK  output_shape={loss.shape}")
    print(f"  loss value      : {loss.item():.6f}")
    print(f"  TCKD component  : {loss_tckd_only.item():.6f}")
    print(f"  NCKD component  : {loss_nckd_only.item():.6f}")
    print(f"  Linearity check : PASSED")
...
def cross_entropy_loss(student_logits: torch.Tensor,
                       labels: torch.Tensor) -> torch.Tensor:
    """
    Standard cross-entropy loss between student predictions and ground-truth labels.
    
    Args:
        student_logits: (B, C) student logits (unnormalized)
        labels: (B,) ground truth class indices
...
if __name__ == "__main__" or True:
    ...
    # Verify against nn.CrossEntropyLoss
    ce_module = nn.CrossEntropyLoss()
    expected = ce_module(student_logits, labels)
    assert torch.allclose(loss, expected, atol=1e-6), \
        f"Mismatch with nn.CrossEntropyLoss: {loss.item()} != {expected.item()}"
    
    print(f"ALGO_CHECK Cross-Entropy Loss: OK  output_shape={loss.shape}")
    print(f"  loss value: {loss.item():.6f}")
...
def total_loss(student_logits: torch.Tensor,
               ...
               alpha: float = 1.0,
               beta: float = 8.0,
               ce_weight: float = 1.0) -> torch.Tensor:
    """
    Total training loss for DKD (Decoupled Knowledge Distillation).
    
    total_loss = ce_weight * CE_loss + DKD_loss
    where DKD_loss = (alpha * TCKD + beta * NCKD) * T^2
    
    Note: The T^2 scaling is already applied inside dkd_loss, so we just
    combine CE and DKD here with their respective weights.
    
    Args:
        student_logits: (B, C) student logits
        teacher_logits: (B, C) teacher logits
        labels: (B,) ground truth class indices
        T: temperature for distillation (default: 4.0)
        alpha: weight for TCKD component (default: 1.0)
        beta: weight for NCKD component (default: 8.0, as in paper for ResNet32x4->ResNet8x4)
    ...
    # TCKD: binary KL on target vs non-target
    student_probs = F.softmax(student_logits / T, dim=1)
    teacher_probs = F.softmax(teacher_logits / T, dim=1)
    ...
    # NCKD: KL among non-target classes
    mask = torch.zeros(batch_size, num_classes, device=student_logits.device, dtype=torch.bool)
    mask[arange, labels] = True
...
if __name__ == "__main__" or True:
    ...
    loss = total_loss(student_logits, teacher_logits, labels, T=T, alpha=alpha, beta=beta)
    
    assert loss.shape == torch.Size([]), f"Expected scalar, got shape {loss.shape}"
    assert loss.item() >= 0, f"Total loss should be non-negative, got {loss.item()}"
    
    # Verify components manually
    ce = F.cross_entropy(student_logits, labels)
    
    # Recompute DKD manually for verification
    arange = torch.arange(B)
    s_probs = F.softmax(student_logits / T, dim=1)
    t_probs = F.softmax(teacher_logits / T, dim=1)
    s_pt = s_probs[arange, labels]
    t_pt = t_probs[arange, labels]
    s_bin = torch.stack([s_pt, 1.0 - s_pt], dim=1).clamp(min=1e-8)
    t_bin = torch.stack([t_pt, 1.0 - t_pt], dim=1).clamp(min=1e-8)
    tckd_val = F.kl_div(torch.log(s_bin), t_bin, reduction='batchmean') * (T ** 2)
    
    mask = torch.zeros(B, C, dtype=torch.bool)
    mask[arange, labels] = True
    neg_inf = torch.full_like(student_logits, float('-inf'))
    s_masked = torch.where(mask, neg_inf, student_logits / T)
    t_masked = torch.where(mask, neg_inf, teacher_logits / T)
    t_phat = F.softmax(t_masked, dim=1)
    s_log_phat = torch.nan_to_num(F.log_softmax(s_masked, dim=1), nan=0.0, neginf=0.0)
    nckd_val = F.kl_div(s_log_phat, t_phat, reduction='batchmean') * (T ** 2)
    
    expected = 1.0 * ce + alpha * tckd_val + beta * nckd_val
    assert torch.allclose(loss, expected, atol=1e-5), \
        f"Total loss mismatch: {loss.item()} != {expected.item()}"
    
    print(f"ALGO_CHECK Total Loss: OK  output_shape={loss.shape}")
    print(f"  total loss value : {loss.item():.6f}")
    print(f"  CE component     : {ce.item():.6f}")
    print(f"  TCKD component   : {tckd_val.item():.6f}")
    print(f"  NCKD component   : {nckd_val.item():.6f}")
    print(f"  DKD component    : {(alpha * tckd_val + beta * nckd_val).item():.6f}")
